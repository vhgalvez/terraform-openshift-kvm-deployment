...
resource "libvirt_pool" "volumetmp_kube_03" {
  name = "${var.cluster_name}_kube_03"
  type = "dir"
  path = "/var/lib/libvirt/images/${var.cluster_name}_kube_03"
}

resource "libvirt_volume" "base" {
  name   = "${var.cluster_name}_kube_03-base"
  source = var.base_image
  pool   = libvirt_pool.volumetmp_kube_03.name
  format = "qcow2"
}
...
data "template_file" "vm-configs" {
  ...
  template = file("${path.module}/config/machine-${each.key}-config.yaml.tmpl")
  vars = {
